...
\begin{enumerate}
    ...
    \item \textbf{Digitization and provenance.} Values read from published figures carry reading error. We did not perform repeat-digitization reliability testing, but we did cross-check against independently stated values, obtaining agreement within about four percentage points (Section~\ref{sec:data}). Caine et al. stomatal-density reductions are taken from that paper's Results text rather than digitized, after we found an earlier version of this dataset had used densities inconsistent with the source (39\%/71\% against the stated 58\%/88\%); we recommend such cross-checks against stated values as routine practice when compiling datasets from published figures.
