% ============================================================================
% LeafLens -- Springer LNCS proceedings format (llncs.cls)
%
% llncs is a standard CTAN/TeX-Live class, so unlike the previous Springer Nature
% journal (sn-jnl) version of this file, NOTHING extra needs to be bundled or
% downloaded: Overleaf and any standard TeX distribution supply it automatically.
% This file has been test-compiled with tectonic and builds cleanly.
%
% Package choices below (newtxtext/newtxmath[varvw] for Times-based text and math,
% rather than the plainer fontenc-only setup used previously) match the current
% LNCS sample chapter (Version 2.25).
% ============================================================================
...
\urlstyle{rm}
%
% Compatibility shim: \ackname, \discintname and the credits environment were added
% to llncs.cls in v2.25 (2026); only define them if the installed class is older and
% lacks them, so this works whether Overleaf/TeX Live has the current class or not.
\makeatletter
\providecommand{\ackname}{Acknowledgments}
\providecommand{\discintname}{Disclosure of Interests}
\@ifundefined{credits}{%
  \newenvironment{credits}{\par\addvspace{1.5em}\small}{\par\addvspace{0.5em}}%
}{}
\makeatother
%
\begin{document}
...
Two Karavolias et al. (2024) promoter alleles (Alleles 5 and 6) carry \emph{higher} stomatal density than wild type; this is a genuine hypermorphic phenotype reported by that study and both are retained. For Caine et al. (2019) we take the stomatal-density reductions directly from that paper's Results text (58\% for \textit{OsEPF1oe}W and 88\% for \textit{OsEPF1oe}S relative to IR64 controls), since only the ratio to wild type enters the model.

We validated the digitization against values stated independently in the source texts. Digitized Karavolias et al. (2024) promoter alleles span 71.5--118.0\% of wild-type density against that paper's stated range of 70--120\%, and its \textit{stomagen} knockout digitizes to 78.6\% reduction against a stated 80\%. Digitized Karavolias et al. (2023) lines give 21.3\% and 83.3\% of wild-type density against stated approximate values of 25\% and 80\%. Agreement is therefore within about four percentage points throughout. As a further independent check, the digitized Karavolias et al. (2024) genotype means reproduce the linear relationships that study reports in its Fig.~2: stomatal density correlates with conductance at $r=+0.96$, with assimilation at $r=+0.95$, and with intrinsic water-use efficiency at $r=-0.90$, matching the reported strong positive, positive and negative associations respectively.

Each record includes stomatal density (SD), wild-type density, relative reduction (\%), net photosynthetic rate ($A$), stomatal conductance ($g_s$), intrinsic WUE ($A/g_s$), ambient CO$_2$, PAR, temperature, relative humidity, water treatment, and an \texttt{A\_Source} flag (\texttt{digitized}, \texttt{digitized\_genotype\_mean}, or \texttt{estimated\_regression}). Figure~\ref{fig2} shows the raw relationships between stomatal reduction and $A$/$g_s$. A relative-reduction histogram (not separately plotted) shows the combined dataset has essentially no measurements between 40\% and 70\% reduction -- values cluster near 0--30\% and again near 70--80\%, a genuine gap in the underlying experiments rather than an artifact of our processing, and one we return to as a concrete recommendation for future work (Sect.~\ref{sec:discussion}).

\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/fig_scatter_a_gs.png}
\caption{Corrected raw data from the three studies. \textbf{Left:} net photosynthetic rate ($A$) vs. stomatal density reduction. \textbf{Right:} stomatal conductance ($g_s$) vs. stomatal density reduction. Colors indicate study source.} \label{fig2}
\end{figure}

\subsection{Regional Climate Data}\label{sec:climate}

We retrieved 11.5 years (Jan 2015--Jun 2026) of daily NASA POWER climate data~\cite{ref_nasa} for five Telangana districts: Warangal, Nizamabad, Karimnagar, Nalgonda, and Khammam. Daily air temperature and relative humidity were used to compute VPD via the Tetens/Murray approximation of saturation vapor pressure as presented in Allen et al. (1998, FAO Irrigation \& Drainage Paper 56, Eq.~11)~\cite{ref_fao56}:
\begin{equation}
e_s(T) = 0.6108 \times \exp\!\Bigl(\frac{17.27\,T}{T + 237.3}\Bigr),\qquad
\text{VPD} = e_s(T)\times\Bigl(1 - \frac{\text{RH}}{100}\Bigr).
\end{equation}
PPFD was estimated from shortwave radiation as $\mathrm{PPFD} = R_s\times10^{6}\times f_{\mathrm{PAR}}\times q / D$, with PAR fraction $f_{\mathrm{PAR}}=0.45$~\cite{ref_mccree}, quantum conversion $q=4.6\,\mu$mol\,J$^{-1}$, and assumed photoperiod $D=43\,200$\,s, giving a daytime-average PPFD of 51--1351\,$\mu$mol\,m$^{-2}$\,s$^{-1}$. Figure~\ref{fig3} shows the monthly VPD distribution; VPD is highest in April--May, with district-seasonal means reaching 3.39\,kPa and isolated daily maxima up to 5.7\,kPa.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/fig_vpd_monthly.png}
\caption{Monthly VPD distribution across five Telangana districts (2015--2026), colored by season (Rabi/winter, Pre-Monsoon, Kharif/monsoon). VPD peaks in April--May.} \label{fig3}
\end{figure}
...
\end{equation}
with $\mathrm{MS}_B$, $\mathrm{MS}_W$ the between- and within-group mean squares and $n_0$ the standard unbalanced-design correction to average group size~\cite{ref_searle}. We compute Eq.~\ref{eq:ceiling} for three candidate targets and select on that basis (Sect.~\ref{sec:whyceiling}).
...
\subsection{Robustness Analyses}\label{sec:robustness}

Two threats to the headline estimates are addressed quantitatively rather than by disclaimer.

\textbf{Digitization error.} Because all gas-exchange values are read from published figures, reading error is a potential source of bias. We measure it directly: the $x$-axis of the Karavolias et al. (2023) light-response figures corresponds to PAR levels stated numerically in that paper's text, so the deviation of each digitized $x$-reading from its nominal level is pure reading error. We then inject Gaussian noise into every digitized quantity (assimilation, conductance and stomatal density) at and well beyond the measured precision, and re-run the full modelling pipeline, to establish whether conclusions depend on digitization quality.

\textbf{Selection optimism.} Model class, feature set and regularization strength were chosen by comparing validation scores on this dataset, which biases the resulting $R^{2}$ upward. We quantify that bias with nested cross-validation: the entire selection procedure -- a search over 24 configurations (physics prior on/off $\times$ ridge/XGBoost $\times$ three feature sets $\times$ three ridge penalties) -- is executed inside an inner loop on outer-training data only, and the selected configuration is scored on an outer fold that informed no choice. The difference between the standard and nested estimates is the selection optimism, measured rather than asserted. The choice of \emph{target} ($A$ and $g_s$ rather than $A/g_s$) is not nested, since $R^{2}$ is not comparable across different response variables; that decision rests on the variance-ceiling analysis, which is computed from replicate structure alone without fitting any model and therefore consumes no validation information. Finally, a permutation test re-runs the pipeline against shuffled targets to establish what the procedure can manufacture from noise.

\textbf{Imputed values, functional form and physiological plausibility.} Three further checks probe specific modelling commitments. We test whether the 59 regression-estimated assimilation values inflate performance, by scoring out-of-fold predictions separately on imputed and measured rows within the same model, and by refitting on measured rows only alongside a sample-size-matched random subset that retains drought coverage. We re-run the pipeline with two alternative saturating forms for the physics prior (exponential and hyperbolic) in place of the Michaelis--Menten form. And because no source paper states absolute assimilation or conductance values numerically -- so the $y$-axis cannot be cross-checked the way the $x$-axis was (Sect.~\ref{sec:data}) -- we instead check all digitized values against established physiological ranges for C3 rice.

\section{Results}\label{sec:results}

We first establish which response variables are learnable at all, then report predictive performance and the evidence that it is not an artifact of validation design or model selection. We then give the biological consequence -- the carbon--water trade-off -- and close with the three constraints that bound how far it can be extrapolated.

\subsection{Which Targets Are Learnable}\label{sec:whyceiling}

Table~\ref{tab2} applies Eq.~\ref{eq:ceiling} to three candidate targets. Assimilation and conductance are largely learnable; their ratio is not. For intrinsic water-use efficiency the ICC is $0.000$: after correctly discounting singleton-cell replicates, there is no between-genotype variance left to distinguish from noise. The effect size makes the reason concrete: across genotypes, mean $A/g_s$ relative to wild type spans 0.96--1.29 (standard deviation 0.087), while the mean within-genotype replicate standard deviation is 0.238 -- the noise is 2.7$\times$ larger than the entire biological signal. A ratio of two independently noisy measurements compounds the noise of both.

\begin{table}
\caption{Maximum attainable $R^{2}$ for each candidate target, via the ANOVA ICC (Eq.~\ref{eq:ceiling}).}\label{tab2}
\centering
\small
\begin{tabular}{lcc}
\toprule
...
The paired comparison isolates the contribution of the prior on identical folds (Table~\ref{tab4}). The physics prior improves $R^{2}$ by $+0.116\pm0.091$ for assimilation and $+0.042\pm0.023$ for conductance, and wins in 20 of 20 repeats for both targets, so the benefit is reproducible rather than an artifact of a favourable split. Figure~\ref{fig5} shows all variants against their ceilings, and Figure~\ref{fig6} shows out-of-fold predictions against observations.

\begin{table}
\caption{Paired benefit of the physics prior, evaluated on identical folds.}\label{tab4}
\centering
\small
\begin{tabular}{lccc}
\toprule
...
\caption{Genotype-grouped CV $R^{2}$ for all model variants, with error bars over 20 randomized fold assignments. Dashed green lines mark the variance ceiling (Table~\ref{tab2}).} \label{fig5}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/fig_pred_vs_obs.png}
\caption{Out-of-fold predictions versus observations for assimilation (left) and conductance (right), colored by source study. Dashed line is 1:1.} \label{fig6}
\end{figure}
...
\subsection{Robustness: Digitization Error and Selection Optimism}\label{sec:robustnessresults}

\textbf{Digitization precision is high and does not limit the conclusions.} Across 60 digitized points whose true $x$-values are stated in the source text, reading error averages 0.19\% of the axis span, with a 95th percentile of 0.41\% and a maximum of 0.45\%. Injecting Gaussian noise at that measured level leaves performance unchanged ($R^{2}=0.496$ for $A$ against an unperturbed 0.495; $0.337$ for $g_s$ against 0.314). Performance degrades only slowly thereafter: at 5\% noise -- more than ten times the measured error -- $R^{2}$ for $A$ is still 0.447, and at 10\% noise, roughly twenty-five times the measured error, it remains 0.375. Digitization quality is therefore not a material threat to these results.

\textbf{Selection optimism is present but modest, and the architecture survives blind reselection.} Nested cross-validation gives unbiased estimates of $R^{2}=0.428\pm0.644$ for $A$ and $0.173\pm0.573$ for $g_s$, against standard estimates of $0.495$ and $0.314$ computed identically but without nesting. The implied selection optimism is therefore $+0.07$ for $A$ and $+0.14$ for $g_s$: real, larger for the noisier target, and small enough that both nested estimates remain positive. Readers should discount the headline values in Table~\ref{tab3} by approximately this much. Notably, the inner selection loop -- which has no knowledge of our modelling choices -- independently selected the physics-informed ridge architecture in the majority of outer folds (28 of 47 for $A$, 26 of 46 for $g_s$, with the remainder split across XGBoost variants and other ridge penalties), indicating that the architecture was not an artifact of our search.

\textbf{The signal is not manufactured by the selection procedure.} Running the identical pipeline against permuted targets yields a null distribution centred near zero (mean $R^{2}=-0.11$ for $A$, $-0.09$ for $g_s$) whose 95th percentile is approximately $0.00$ for both. No permutation of 30 reached the observed performance for either target ($p<0.001$).

\textbf{Imputed values do not inflate performance; they dilute it.} Within the same model, out-of-fold prediction is markedly \emph{worse} on the 59 regression-estimated rows ($R^{2}=0.03$ for $A$, $0.03$ for $g_s$) than on the 106 measured rows ($0.81$ and $0.54$). The imputation therefore adds variance the model cannot fit rather than manufacturing artificial predictability, which is the failure mode that would have been disqualifying. Refitting on measured rows only does lower performance for $A$ ($R^{2}=-0.31$), but this is attributable to experimental coverage rather than to imputation: every drought measurement in the dataset is an imputed row, so the measured-only subset is well-watered exclusively and the drought indicator becomes constant. A sample-size-matched random subset that retains drought coverage scores $+0.42$, against $-0.31$ for the drought-free measured subset. The honest reading is that the $A$ model depends on having more than one water treatment represented, not that it depends on imputed values.

\textbf{Conclusions do not depend on the chosen functional form.} Substituting an exponential or hyperbolic saturation term for the Michaelis--Menten form changes $R^{2}$ by less than 0.03 ($A$: 0.493, 0.517, 0.517; $g_s$: 0.340, 0.346, 0.345). The alternatives are in fact marginally better, so the Michaelis--Menten form was not selected for performance; it was chosen as the standard description of light-limited assimilation, and the choice is immaterial to the result.

\textbf{Digitized values are physiologically plausible.} No source paper states absolute assimilation or conductance values numerically, so the $y$-axis cannot be validated against stated values as the $x$-axis was. As a substitute check, all 165 rows fall within established ranges for C3 rice: assimilation at saturating light spans 9.9--30.4\,$\mu$mol\,m$^{-2}$\,s$^{-1}$ (lower values from severe knockouts), conductance 0.035--0.494\,mol\,m$^{-2}$\,s$^{-1}$, and median intrinsic water-use efficiency 72\,$\mu$mol\,mol$^{-1}$; assimilation at PPFD $\le100$ is 1.7--4.7, correctly light-limited. No row lies outside physiologically plausible bounds.
...
\begin{table}
\caption{Predicted carbon--water trade-off within the validated domain.}\label{tab6}
\centering
\small
\begin{tabular}{lcc}
\toprule
...
At 30\% reduction the model predicts retention of 94.9\% of wild-type assimilation against 92.0\% of conductance. Figure~\ref{fig8} plots both curves, with the unmeasured 40--70\% range shaded. We deliberately do not nominate a single optimal reduction: as Sect.~\ref{sec:gap} shows, the range in which an optimum would most plausibly lie contains no measurements at all.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/fig_tradeoff.png}
\caption{Predicted carbon gain and water loss as a function of stomatal density reduction, within the validated domain (left) and extrapolated to Telangana pre-monsoon conditions (right). Shaded band marks the 40--70\% range, where no published measurements exist.} \label{fig8}
\end{figure}
...
\subsection{Model Class Should Follow From Grouping Structure}
Gradient-boosted trees underperformed ridge regression on every comparison here, often substantially and always with much greater variance (Table~\ref{tab3}), for the structural reason given in Sect.~\ref{sec:model}. The generalizable point is that the appropriate model class is dictated less by sample size than by what the grouping structure asks the model to do. Where groups are defined by distinct values of the principal continuous predictor -- as they are whenever genotypes are distinguished by the very trait being engineered -- held-out prediction is an interpolation problem, and a learner that fits piecewise-constant functions of that predictor cannot solve it however much data it is given. Practitioners in this setting often reach for flexible ensembles on the reasoning that biological responses are nonlinear; our results suggest checking first whether the validation design makes smoothness in the grouping variable a hard requirement rather than a stylistic preference. Notably, the nested selection loop reached the same conclusion independently, choosing a ridge residual in the majority of outer folds (Sect.~\ref{sec:robustnessresults}).

\subsection{Limitations}\label{sec:limitations}
\begin{enumerate}
    \item \textbf{Reliability degrades with reduction severity.} Leave-one-genotype-out $R^{2}$ correlates negatively with reduction magnitude ($\rho=-0.50$ to $-0.58$), and severity is partly confounded with source study (Sect.~\ref{sec:genoreliability}). Predictions above roughly 30\% reduction should be treated as illustrative, not validated.
    \item \textbf{Imputed assimilation values.} 59 of 165 rows use regression-estimated rather than measured $A$ ($R^{2}=0.931$ on 8 genotype means) and are flagged in the released data. They are not independent measurements. They do not, however, inflate apparent fit: the model predicts them substantially worse than the measured rows (Sect.~\ref{sec:robustnessresults}). The associated constraint is different and more limiting -- every drought measurement in the dataset is an imputed row, so drought coverage and imputation cannot be separated.
    ...
    \item \textbf{Digitization and provenance.} Values read from published figures carry reading error, measured here at 0.41\% of axis span (95th percentile) and shown not to affect conclusions until roughly twenty-five times that level (Sect.~\ref{sec:robustnessresults}). We did not perform independent repeat digitization by a second operator, which would additionally capture operator-level bias rather than reading precision alone. Caine et al. stomatal-density reductions are taken from that paper's Results text rather than digitized, after we found an earlier version of this dataset had used densities inconsistent with the source (39\%/71\% against the stated 58\%/88\%); we recommend such cross-checks against stated values as routine practice when compiling datasets from published figures.
    \item \textbf{Selection over modelling choices.} Model class, feature set and regularization strength were chosen by comparing validation scores within this dataset. Nested cross-validation quantifies the resulting optimism at $+0.07$ ($A$) and $+0.14$ ($g_s$), and the headline values should be discounted accordingly (Sect.~\ref{sec:robustnessresults}). The choice of target variable is not nested and rests instead on the variance-ceiling analysis, which consumes no validation information; it is nonetheless a choice made with knowledge of this dataset, and an independent replication would be required to rule out subtler forms of dataset-specific tailoring.
\end{enumerate}

\subsection{Future Work}
The most valuable next step is experimental rather than computational: genotypes spanning 40--70\% stomatal-density reduction, phenotyped across a temperature and VPD range that overlaps field conditions, with a wild-type control in every environment. Such a dataset would convert the extrapolations in Figure~\ref{fig8} into interpolations and permit the optimum this framework currently declines to nominate. On the modelling side, a hierarchical formulation with study-level random effects would use the cross-study structure more efficiently than post-hoc normalization, and coupling validated $A$ and $g_s$ predictions to a canopy or crop-growth model would connect leaf-level trade-offs to season-scale yield and water budgets.
...
The authors thank the developers of the NASA POWER API for open-access climate data. Source code and data are available at \url{https://github.com/3rishna/LeafLens}. All digitized data, processing scripts and outputs are available at the repository above. Portions of the data pipeline's debugging, re-verification, and manuscript editing were assisted by an AI coding assistant (Claude, Anthropic); all scientific claims and conclusions were reviewed and are the responsibility of the authors.
